 ...
 */
var edBoxWeb = angular.module("edBoxWeb", []);
function getNgScope() {
 ...
 * API Abstraction
 */
function api(apiConnectType, url, data, asyncMode, callback, feedBack = false, loadingNotice = null) {
    $.ajax({
        type: apiConnectType,
        url: url,
        async: asyncMode,
    ...
    .success(function (remoteData) {
        if (remoteData.Status === true) {
            ...
            swalWarning(remoteData.Message); hideLoader(loadingNotice);
        }
    })
    .error(function () {
        swalEx(); hideLoader(loadingNotice);
    });
};

function hideLoader(loadingNotice) {
